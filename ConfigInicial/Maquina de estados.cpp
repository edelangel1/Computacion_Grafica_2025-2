/*
Animacion por Maquinas de Estados
Entrega: 22/04/2025
...
int main(){
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion maquina de estados - J. Eduardo Perez Del Angel", nullptr, nullptr);
...
void Animation() {
	...
	if (dogAnim == 1) {		// Walk Animation
		...
		// Definir el límite del piso 
		const float floorEnd = 2.3f; // El perro se detendrá antes de llegar a esta posición en Z

		// Solo animar si no se ha llegado al límite
		if (dogPos.z < floorEnd) {
			if (!step) {		// State 1
				RLegs += 0.3f;
				FLegs += 0.3f;
				head += 0.3f;
			...
			dogPos.z += 0.001; // Velocidad de desplazamiento 
