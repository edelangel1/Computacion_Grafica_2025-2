...
// Variables nuevas para animación vertical
float ballHeight = 0.5f;  // Comienza en la nariz del perro
const float ballMaxHeight = 2.1f; // Altura máxima (nivel de la luz)
const float ballMinHeight = 0.5f; // Altura mínima (nariz del perro)
...
int main(){
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion basica - Eduardo Perez", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window)){
		...
		model = glm::translate(model, glm::vec3(0.0f, ballHeight, 0.0f)); //
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void Animation() {
	...
	if (ballMoving) {
		if (goingDown) {
			ballHeight -= ballSpeed * deltaTime;  // Desciende la pelota si goingDown es true
			...
			ballHeight += ballSpeed * deltaTime;  // Sube la pelota si goingDown es false
