...
//
//// Cube 2
//glm::vec3 lightPos2(-1.0f, -0.5f, 2.5f);
//float movelightPos2 = 1.0f;              
//GLfloat deltaTime2 = 0.0f;
//GLfloat lastFrame2 = 0.0f;
//float rot2 = 45.0f;                      
//bool activanim2 = true;
...
int main(){
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Materiales e Iluminacion - Joaquin Eduardo Perez Del Angel", nullptr, nullptr);
    ...
    // Load models
    Model dog((char*)"Models/Banca/Bench.obj");
    ...
    image = stbi_load("Models/Banca/Diffuse_Bake.jpg", &textureWidth, &textureHeight, &nrChannels, 0);
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        // Set lights properties 1
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light.ambient"), 0.3f, 0.3f, 0.3f); 
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light.diffuse"), 0.8f, 0.7f, 0.8f);
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light2.diffuse"), 9.8f, 9.7f, 9.8f);
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "material.specular"), 1.0f, 0.0f, 0.0f); 
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode){
    ...
    // Controls cube 2
    if (keys[GLFW_KEY_I]) {
        lightPos2.y += 0.1f; // Mueve el cubo hacia arriba en el eje Y
    }

    if (keys[GLFW_KEY_K]) {
        lightPos2.y -= 0.1f; // Mueve el cubo hacia abajo en el eje Y
    }
}
